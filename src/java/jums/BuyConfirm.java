...
public class BuyConfirm extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            /*非ログイン状態でのアクセスに対してLoginへ飛ばし、非ログイン状態であることを知らせる*/

            Log.getInstance().LogWrite("BuyConfirmにアクセスされました");
            if (session.getAttribute("userID") == null || session.getAttribute("userID").equals("0")) {
                Log.getInstance().LogWrite("ログイン状況を確認できませんでした。Loginに戻ります");
                request.setAttribute("error", "ログインされていません");
                request.getRequestDispatcher("/Login").forward(request, response);
            }
            Log.getInstance().LogWrite("buyconfirmに遷移します");
